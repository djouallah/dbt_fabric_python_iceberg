...
**Data refresh is manual.** [`data.yml`](.github/workflows/data.yml) is `workflow_dispatch` only — the hourly cron was removed once [`analytics-as-code`](https://github.com/djouallah/analytics-as-code) took over unattended loading (every 30 minutes, into its own lakehouse in the same workspace); two scheduled writers in one workspace is capacity spent twice. So this lakehouse and its Direct Lake semantic model refresh only when you dispatch `data.yml` or `pipeline.yml`. Re-add `schedule: - cron: '0 * * * *'` to bring the automatic refresh back. A dispatch fetches up to 60 files per feed per run, and both limits are overridable, along with two maintenance inputs: `compact_only` (skip the load, just compact) and `min_input_files` (how many data files a table needs before it is worth rewriting; drop it to `2` to force a rewrite). The `compact` job runs after every load and is `continue-on-error` — maintenance never fails the pipeline, and it is deliberately not gated on the load succeeding, since fragmentation is just as real after a failed build.
